import { Injectable } from "@angular/core";

export interface Student {
    lastName: string;
    firstName: string;
    middleName: string;
    birthDate: string;
    averageScore: string;

}

@Injectable({ providedIn: "root" })
export class DataService {
    private data: Student [] = [
        {
            lastName:"Фадеева",
            firstName: "Анастасия",
            middleName:"Ивановна",
            birthDate: "1998-04-20",
            averageScore: "3.7"

        },
        {
            lastName:"Левченко",
            firstName: "Яна",
            middleName:"Александровна",
            birthDate: "1998-06-07",
            averageScore: "3.9"

        },
        {
            lastName:"Число",
            firstName: "Руслан",
            middleName:"Викторович",
            birthDate: "1998-10-24",
            averageScore: "4.5"

        },
        {
            lastName:"Ромашова",
            firstName: "Анастасия",
            middleName:"Александровна",
            birthDate: "1998-04-05",
            averageScore: "3.5"

        },
        {
            lastName:"Федосеева",
            firstName: "Татьяна",
            middleName:"Дмитриевна",
            birthDate: "1997-09-02",
            averageScore: "4.5"

        },
        {
            lastName:"Ильющенкова",
            firstName: "Татьяна",
            middleName:"Алексеевна",
            birthDate: "1998-11-24",
            averageScore: "5"

        },
        {
            lastName:"Соловьева",
            firstName: "Екатерина",
            middleName:"Викторовна",
            birthDate: "1998-08-17",
            averageScore: "2.5"

        },
        {
            lastName:"Лосяков",
            firstName: "Алексей",
            middleName:"Александрович",
            birthDate: "1998-01-15",
            averageScore: "3.7"

        },
        {
            lastName:"Малов",
            firstName: "Дмитрий",
            middleName:"Иванович",
            birthDate: "1998-05-24",
            averageScore: "4.0"

        },
        {
            lastName:"Котихина",
            firstName: "Екатерина",
            middleName:"Дмитриевна",
            birthDate: "1998-03-07",
            averageScore: "3.5"

        },
        {
            lastName:"Беляев",
            firstName: "Антон",
            middleName:"Павлович",
            birthDate: "1998-11-13",
            averageScore: "2.7"

        },
        {
            lastName:"Жакевич",
            firstName: "Мария",
            middleName:"Андреевна",
            birthDate: "1999-07-20",
            averageScore: "4.2"

        },
        {
            lastName:"Сухарев",
            firstName: "Вячеслав",
            middleName:"Алексеевич",
            birthDate: "1998-04-02",
            averageScore: "4.0"

        },
        {
            lastName:"Прохорова",
            firstName: "Кристина",
            middleName:"Ивановна",
            birthDate: "1997-10-20",
            averageScore: "5"

        },
        {
            lastName:"Борисов",
            firstName: "Роман",
            middleName:"Анатольевич",
            birthDate: "1998-07-16",
            averageScore: "3.5"

        },
        {
            lastName:"Шрайнер",
            firstName: "Диана",
            middleName:"Ивановна",
            birthDate: "1998-12-23",
            averageScore: "4.2"

        },
        {
            lastName:"Галиеев",
            firstName: "Альберт",
            middleName:"Андреевич",
            birthDate: "1998-11-24",
            averageScore: "2.7"

        },
        {
            lastName:"Цибикова",
            firstName: "Елена",
            middleName:"Алесандровна",
            birthDate: "1998-09-14",
            averageScore: "3.9"

        },
        {
            lastName:"Шепарский",
            firstName: "Дмитрий",
            middleName:"Дмитриевич",
            birthDate: "1998-06-17",
            averageScore: "4.5"

        },
        {
            lastName:"Батрасова",
            firstName: "Татьяна",
            middleName:"Вячеславовна",
            birthDate: "1998-10-13",
            averageScore: "4.2"

        },
        {
            lastName:"Кузина",
            firstName: "Алина",
            middleName:"Александровна",
            birthDate: "1998-02-14",
            averageScore: "5"

        },
        {
            lastName:"Гусева",
            firstName: "Дарья",
            middleName:"Анатольевна",
            birthDate: "1998-03-20",
            averageScore: "5"

        },
        {
            lastName:"Тюрина",
            firstName: "Виктория",
            middleName:"Ивановна",
            birthDate: "1998-08-03",
            averageScore: "4.5"

        },
    ];

    private hiddenFilter: boolean = true;

    // do smth with data information
    getData(): Student[] {
        return this.data;
    }

    addData(items: Student): void {
        this.data.push(items);
    }

    deleteData(index: number): void {
        this.data.splice(index, 1);
    }

    // filter variables
    getFilterFalg(): boolean {
        return this.hiddenFilter;
    }
    setFilterFlag(flag: boolean): boolean {
        this.hiddenFilter = flag;
        return this.hiddenFilter;
    }

    setFilterList(list: Student[]): void{
        this.data = list;
    }


}
